/*
]=====> XLaw CD <=====[ ]=====> YT SC∆R ID <=====[ ]=====> 083821123163 <=====[
*/
const {
   WAConnection,
   MessageType,
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:XLAW ID\n' 
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6283821123163:+62 838-2112-3163\n'
            + 'END:VCARD'

const ngonsol = JSON.parse(fs.readFileSync('./settings/xlaw.json'))
const {
    botName,
    ownerName,
    XteamKey,
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SUBSCRIBE YT SC∆R ID'))
})

	baby.on('credentials-updated', () => {
		fs.writeFileSync('./xlaw.json', JSON.stringify(baby.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ingfokan cuyy...')
	})
	fs.existsSync('./xlaw.json') && baby.loadAuthInfo('./xlaw.json')
	baby.on('connecting', () => {
		start('2', 'XLaw Connecting...')
	})
	baby.on('open', () => {
		success('2', 'XLaw Connected')
	})
...
*/
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis XLaw baik hati`)
				nul = body.slice(7)
				...
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis1 XLaw baik hati`)
				xlaw = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				buff = await getBuffer(`https://api.xteam.xyz/magernulis2?text=${xlaw}&APIKEY=${XteamKey}`)
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis2 XLaw baik hati`)
				laysha = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo XLaw & Gans`)
				reply(nad.wait())
		...
		case 'halloweentext':
		    ...
		    break
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub XLaw & Hub`)
				reply(nad.wait())
		...
		case 'textlight':
		    ...
		    break
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}glitchtext XLaw & Gans`)
					reply(nad.wait())
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, 'Kode bahasanya mana kak? contoh : ${prefix}tts id Halo XLaw', text, {quoted: mek})
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata XLaw`
...
case 'neontext':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}neontext XLaw`)
					neon = body.slice(10)
					reply('「❗」WAIT GANS')
...
*/		 
				...
				if (!isRegistered) return reply(nad.noregis())
					if (isLimit(sender)) return reply(nad.limitend(pusname))
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}blackpink XLaw`)
					pink = body.slice(11)
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner XLaw`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena XLaw baik Bot memberikan ${one}Xp >_<`)
                      }else{
...
if (!isRegistered) return reply(nad.noregis())
if (isLimit(sender)) return reply(nad.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}hartatahta XLaw`)
har = body.slice(12)
...
case 'cloudtext':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}cloudtext XLaw`)
cloud = body.slice(11)
